...
- TUI: single task with `tokio::select!` over `stdin.read`, `read_envelope`, `winch.recv`, and a 30 Hz tick. The loop dispatches to the pure-state-machine `App::handle_event` which emits an ordered `Vec<AppAction>`; the runtime executes them. The view is a fixed tiled layout per `docs/DECISIONS.md#7`: all scopes are visible simultaneously and all subscriptions (`AttachPane`, `Subscribe(Docker)`, and later Ports / Fleet / ClaudeCode) live concurrently for the life of the TUI process. Focus (`Ctrl-b h/j/k/l` + arrows) routes keystrokes to the focused tile; unfocused tiles continue to update live. The pty tile feeds raw bytes through a `vt100::Parser` owned by the App; the parser's screen buffer renders as a ratatui widget within the tile's `Rect` — raw stdout passthrough is gone. Detach (`Ctrl-b d` / `Ctrl-b q`) tears down all subscriptions; reattach re-subscribes and the daemon replays each subscription's snapshot (`PaneSnapshot` for panes, first `ContainerList` for Docker, etc.).

  **Scope rendering contract.** Every scope renderer takes `(state, Frame, Rect)` and draws only within its `Rect`; no renderer owns the full frame. Phases 4 (Ports/Processes), 5 (SSH remote pty), 7 (port scanner), and 9 (Claude Code) inherit this contract — their tile slots into the god-view layout at a labeled placeholder during development and is replaced by the live renderer when the phase lands.
