...
- TUI: single task with `tokio::select!` over `stdin.read`, `read_envelope`, `winch.recv`, and a 30 Hz tick (gated off in Pane mode). The loop dispatches to the pure-state-machine `App::handle_event` which emits an ordered `Vec<AppAction>`; the runtime executes them. Pane mode is raw stdout passthrough (no ratatui draw cycle); Scope mode drives ratatui via `AppAction::DrawScope`. Mode switches cancel the previous `AttachPane` subscription and send a fresh one so the daemon replays scrollback as `PaneSnapshot` (byte-level invariant verified by `vim_preservation.rs`).
