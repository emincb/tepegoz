...
- `tui` — interactive client; god-view tiled layout per `docs/DECISIONS.md#7` (pty tile + scope tiles, all visible simultaneously, focus moves between tiles)
...
- Daemon:
  ...
  - Per-subscription forwarder task for each active subscription. **Subscription uniformity**: every per-client subscription — whether `AttachPane`, `Subscribe(Docker)`, `Subscribe(DockerLogs)`, or `Subscribe(DockerStats)` — lives in a `HashMap<id, AbortHandle>`. `Unsubscribe { id }` cancels across both `pane_subs` and `docker_subs` with a single check. Phase 4 (Ports/Processes), 5 (SSH remote pty), and 7 (port scanner) inherit this shape — every new subscription kind adds a map alongside, not a new mechanism. (Through Slice C1 `pane_subs` was a `JoinSet<()>` with no per-id key; `Unsubscribe` of a pane sub was a silent no-op. The C1 TUI's synthetic re-attach leaked one zombie forwarder per Scope→Pane mode switch until fixed at `43b28eb`. Pinned by `pane_unsubscribe.rs`.)
  - `Subscribe(Docker)` poll task refresh interval 2 s, reconnect interval 5 s; tolerates `dockerd` restarts and engine swaps without re-subscription. `DockerLogs` / `DockerStats` forwarders always emit a terminal `Event::DockerStreamEnded` on stream end (engine unreachable, container exit, container removal, network error); UI never spins waiting for events that won't come.
...
- TUI: single task with `tokio::select!` over `stdin.read`, `read_envelope`, `winch.recv`, and a 30 Hz tick. The loop dispatches to the pure-state-machine `App::handle_event` which emits an ordered `Vec<AppAction>`; the runtime executes them. The view is a fixed tiled layout per `docs/DECISIONS.md#7`: all scopes are visible simultaneously and all subscriptions (`AttachPane`, `Subscribe(Docker)`, and later Ports / Fleet / ClaudeCode) live concurrently for the life of the TUI process. Slice 6.0 documented surface: `Tab` / `Shift-Tab` cycle tile focus in a fixed order (PTY → Docker → Ports → Fleet → ClaudeCode → PTY); arrow keys / `j` / `k` navigate rows inside a focused scope; `Enter` fires the primary action on the selected row (Fleet → open remote pane); `Esc` cancels / backs out; `Ctrl-b d` detaches; `Ctrl-b ?` toggles a centered help overlay that is the authoritative reference. Mouse is enabled via crossterm's SGR capture; the `mouse::MouseParser` strips `\x1b[<…M/m` sequences off the stdin byte stream before `InputFilter` sees it and emits `AppEvent::MouseClick { x, y }` / `AppEvent::MouseHover { x, y }`; click focuses the tile + selects the row; double-click inside 400 ms fires the primary action; hover lights the tile border in cyan-dim. `Ctrl-b h/j/k/l` + arrow spatial focus and `Ctrl-b &` close-active-pane remain as undocumented aliases. The pty tile feeds raw bytes through a `vt100::Parser` owned by the App; the parser's screen buffer renders as a ratatui widget within the tile's `Rect` — raw stdout passthrough is gone. The pty tile's top row is a click-aware tab strip: `[N label*]` per pane + a `[×]` close-active affordance at the tail. Detach (`Ctrl-b d`) tears down all subscriptions; reattach re-subscribes and the daemon replays each subscription's snapshot (`PaneSnapshot` for panes, first `ContainerList` for Docker, etc.).

  **Scope rendering contract** (Slice 6.0 extended). Every scope renderer takes `(state, Frame, Rect, focused: bool, hovered: bool)` and draws only within its `Rect`; no renderer owns the full frame. Border style is picked by `scope::border_style(focused, hovered)`: focused → bright cyan; unfocused + hovered → cyan + DIM; idle → dark gray + DIM. Phases 4 (Ports/Processes), 5 (SSH remote pty), 7 (port scanner), and 9 (Claude Code) inherit this contract — their tile slots into the god-view layout at a labeled placeholder during development and is replaced by the live renderer when the phase lands.
