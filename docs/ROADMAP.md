...
- TUI rewrite: raw-passthrough attacher — raw mode + alt screen, stdin → `SendInput`, `PaneOutput` → stdout, `SIGWINCH` → `ResizePane`. Detach via `Ctrl-b d` (`Ctrl-b q` shipped as an alias here; removed in Slice 6.0).
...
### Slice 5c-ii — FleetAction wire + reconnect UX + red toast · ✅ (`<5c-ii commit>`)

**Delivered.**
- **Wire protocol v8**. `Payload::FleetAction(FleetActionRequest { request_id, alias, kind })` + `Payload::FleetActionResult(FleetActionResult { request_id, alias, kind, outcome })` mirror the `DockerAction` shape for pending-action correlation + toast UX. `FleetActionKind::{Reconnect, Disconnect}`; `FleetActionOutcome::{Success, Failure { reason }}`.
- **Reason propagation** (Option A per CTO review). `Event::HostStateChanged` gains `reason: Option<String>`; populated only on `⚠` terminal states (`AuthFailed` / `HostKeyMismatch` / Phase 6's `AgentNotDeployed` / `AgentVersionMismatch`). `HostState::is_terminal()` helper pinned on the wire type for consumers. `debug_assert!` in `emit_state` guards the invariant that reason only accompanies terminal variants. Codec test verifies both paths round-trip.
- **Per-host action channel**. Coordinator `forward_fleet` keeps `HashMap<alias, mpsc::UnboundedSender<HostAction>>` alongside the supervisor `JoinSet`; every Fleet subscription holds a `FleetSubHandle { abort_handle, action_tx: mpsc::UnboundedSender<FleetActionRequest> }` in `fleet_subs`.
- **Daemon dispatch**. `handle_command`'s new `Payload::FleetAction` arm broadcasts the request to every active Fleet subscription's coordinator (in practice one per client). Coordinator looks up the alias, forwards as `HostAction::{Reconnect, Disconnect}` to the per-host sender, and replies with `FleetActionResult::Success` (dispatched) or `Failure { reason: "unknown alias ..." }` / `Failure { reason: "supervisor task has exited ..." }`.
- **Supervisor state machine rewrite**. Replaces 5c-i's linear `loop { connect; heartbeat; backoff }` with `tokio::select!`-driven arms that all await on `action_rx` alongside their phase future. Reconnect → reset `backoff_idx = 0` + enter Connecting; Disconnect → cancel current phase + drop to idle; terminal states (`AuthFailed`, `HostKeyMismatch`, ProxyJump `AuthFailed`) `await_terminal_reset` on Reconnect so `tepegoz doctor --ssh-forget` + user retry works end-to-end. Connect-future cancellation is handled via `tokio::spawn` + oneshot reply so a mid-flight cancel aborts russh's internal state cleanly (pattern parallels the Phase 4 desync lesson — when cancellation safety is unclear, spawn rather than select-drop).
- **Heartbeat loop**. `run_connected_session` gains an `action_rx` arm alongside its heartbeat timer; Reconnect / Disconnect / Shutdown short-circuit without waiting for the next miss. Returns `ConnectedOutcome::{HeartbeatFailed, ReconnectRequested, DisconnectRequested, Shutdown}`; supervisor branches on the outcome.
- **TUI `Ctrl-b r` keybind** on a focused Fleet row: `dispatch_fleet_reconnect` allocates a `request_id`, registers a `PendingAction { deadline: now + 30 s, description: "reconnect <alias>" }` (same shared timeout sweep as DockerAction), sends `Payload::FleetAction`. Uppercase `R` is an explicit no-op per the capital-discipline rule — reconnect is safe / non-destructive.
- **FleetActionResult correlation**. `handle_daemon_envelope`'s new arm looks up pending action by `request_id`, emits an **Info**-kind toast on Success ("reconnect <alias> — dispatched") — `Info` rather than `Success` because dispatch ≠ completion; the real outcome arrives via `HostStateChanged`. `Failure` emits a red toast with the verbatim reason.
- **Red toast on terminal transitions**. `handle_fleet_event`'s `HostStateChanged` branch checks `state.is_terminal() && reason.is_some() && prev_state != Some(state)` — gates on the transition, not the post-state, so a Reconnect against a ProxyJump host that re-emits `AuthFailed` with the same reason doesn't re-toast. Reason string formats per-state: "<alias>: auth failed — <reason>", "<alias>: host key rejected — <reason>", Phase 6 agent variants.
- **Integration test extension**. The 5c-i container-kill test now also dispatches `FleetAction::Reconnect` with a known `request_id` while the container is still dead (exercising the backoff-retry path); asserts `FleetActionResult::Success` arrives within 5 s. Also exercises the unknown-alias path: sends a `Reconnect` against a bogus alias, asserts `Failure` with a reason string mentioning "unknown alias".
- **Doc gaps folded in** (per 5c-i review):
  - `docs/ARCHITECTURE.md §5` adds a row for SSH host-key TOFU under the default-paths table plus names `TEPEGOZ_CONFIG_DIR` + `TEPEGOZ_DATA_DIR` in the "Overrides" line.
